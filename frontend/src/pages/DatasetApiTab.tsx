...
function DatasetApiTab({API_URL, dataset}: DatasetOverviewTabProps) {
  ...
  const comparisonColumns = dataset.schema.columns.filter((column) =>
    column.operators.some((operator) => [">", ">=", "<", "<="].includes(operator)),
  )
      ...
      <section className="detail-card">
        ...
        <div className="api-parameter-list">
          ...
          {comparisonColumns.length > 0 && (
            <ApiParameter
              name="column_gt, column_gte, column_lt, column_lte"
              description="Compare numeric, date, and datetime columns."
          ...
          <ApiParameter
            name="limit"
            description="Maximum rows to return. Defaults to 100 when omitted or invalid."
            example="?limit=50"
          />
          <ApiParameter
            name="offset"
...
function ApiParameter({
  ...
  return (
    <div className="api-parameter">
      <code>{name}</code>
      <p>{description}</p>
      {example && <span className="parameter-example">Example: <code>{example}</code></span>}
    </div>
  )
}
